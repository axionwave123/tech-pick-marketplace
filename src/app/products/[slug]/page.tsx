  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <nav className="text-sm text-surface-500">
        ...
        {product.categories && (
          <>
            ...
            <Link href={`/categories/${product.categories.slug}`} className="hover:text-brand-600">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-2">
        <div className="relative aspect-square overflow-hidden rounded-3xl border border-surface-200 bg-surface-50">
          {primary ? (
            <Image
              src={primary.url}
        ...
        <div>
          {product.brands && (
            <p className="text-sm font-medium uppercase tracking-wide text-surface-400">{product.brands.name}</p>
          )}
          <h1 className="mt-1 text-3xl font-bold text-surface-900">{product.name}</h1>
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <Rating value={product.avg_rating || 0} size="md" />
            <span className="text-sm text-surface-500">{product.review_count} ratings</span>
            ...
            <div className="mt-6 rounded-2xl border border-surface-200 bg-surface-50 p-5">
              ...
              {best.original_price && best.original_price > best.price && (
                <p className="text-sm text-surface-400">
                  ...
                  <Badge variant="discount" className="ml-2">{formatDiscount(best.original_price, best.price)}</Badge>
                </p>
              )}
              ...
              <p className="mt-3 text-xs text-surface-400">
                Affiliate / outbound links may earn TechPick NG a commission. Prices change on retailer sites.
              </p>
            </div>
          )}
      ...
      {/* Tabs content simplified as sections */}
      <div className="mt-12 grid gap-10 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-10">
          {product.what_stands_out && (
            <section>
              <h2 className="text-xl font-bold text-surface-900">What makes it stand out</h2>
              <p className="mt-3 text-surface-700">{product.what_stands_out}</p>
            </section>
          )}
          ...
          <section className="grid gap-6 sm:grid-cols-2">
            ...
            {product.things_to_consider && product.things_to_consider.length > 0 && (
              <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-5">
                <h3 className="font-semibold text-amber-900">Things to consider</h3>
                <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900/80">
          ...
          {specs.length > 0 && (
            <section>
              <h2 className="text-xl font-bold text-surface-900">Specifications</h2>
              <dl className="mt-4 divide-y divide-surface-100 rounded-2xl border border-surface-200">
                {specs.map((s) => (
                  <div key={s.id} className="grid grid-cols-2 gap-4 px-4 py-3 text-sm">
                    <dt className="text-surface-500">{s.specification_definitions?.label}</dt>
          ...
          {editorial && editorial.status === 'published' && (
            <section className="rounded-2xl border border-surface-200 bg-white p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-bold text-surface-900">TechPick Analysis</h2>
                  <p className="mt-1 text-sm text-surface-500">Editorial summary — based on available public information, not a claim of physical lab testing unless stated.</p>
                </div>
                {editorial.rating != null && <ScoreBadge score={editorial.rating} label="/10" />}
              </div>
              {editorial.summary && <p className="mt-4 text-surface-700">{editorial.summary}</p>}
              {editorial.verdict && (
                <div className="mt-4 rounded-xl bg-surface-50 p-4">
                  <p className="text-sm font-semibold text-surface-900">Verdict</p>
                  <p className="mt-1 text-sm text-surface-700">{editorial.verdict}</p>
                </div>
              )}
            </section>
          )}
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl border border-surface-200 p-5">
            <h3 className="font-semibold text-surface-900">Price comparison</h3>
            <ul className="mt-4 space-y-3">
              {offers.map((o) => (
                <li key={o.id} className="flex items-center justify-between gap-2 text-sm">
                  <div>
                    <p className="font-medium">{o.stores?.name}</p>
                    <p className="text-xs text-surface-400">{relativeTime(o.last_checked_at)}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold">{formatNaira(o.price)}</p>
                    <a
                      href={o.affiliate_url || o.product_url}
                      target="_blank"
                      rel="noopener noreferrer sponsored"
              ...
              {offers.length === 0 && <li className="text-sm text-surface-500">No active offers.</li>}
            </ul>
          </div>
        </aside>
      </div>
